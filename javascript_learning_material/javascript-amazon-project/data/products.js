...
export function getProduct(productId) {
  ...
  products.forEach((product) => { // de-duplicate/normalize the data, by creating a local copy of it
...
// Clothing class will inherit the Product class
class Clothing extends Product { 
  sizeChartLink;

  // if you don't create a constructor for a child class, by default it will run the parent's constructor
  constructor(productDetails) {
  ...
  // crate a size chart link for only clothing products
  // Polymorphism: use a mthod without knowing the class type
  // you can have the class have the same methods, and the class itself will determine what the method does
  extraInfoHTML() { // generate html for size chart. target="_blank" opens the link in a new tab
...
export function loadProductsFetch() {
  ...
  }).catch((error) => { // Error handling for promsies use the catch() method
...
export function loadProducts(func) {
  ...
  // Error handling during a request
  xhr.addEventListener('error', (error) => { // a 'error' value is returned containing details about the error that occurred
    console.log('Unexpected error. Try again later')
  });
...
].map((productDetails) => { // takes each object, save it into a parameter, and runs teh function
  if (productDetails.type === 'clothing') {
    return new Clothing(productDetails);
  }
  return new Product(productDetails); // reutnrs it as a new array
